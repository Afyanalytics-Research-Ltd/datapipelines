# dags/siaya_medical_report_analyzer.py
"""
Deep analysis report for HOSPITALS.SIAYA_MEDICAL_CAMP.SIAYA_MEDICAL_CAMP_COMPLETE_DATA.

Ported from the standalone script `siaya_medical_report_analyzer.py` (repo
root). That table merges two record families fed by
siaya_v2_visits_to_snowflake.py / siaya_v3_visits_to_snowflake.py
(RECORD_TYPE = V2_EMR_VISIT / V3_EMR_VISIT — adult camp visits with
structured vitals, chief complaints, systemic exam fields, etc.) and a
neonatal / PDF-extracted family sharing this table's schema (BABY_NAME,
GESTATIONAL_AGE, APGAR_SCORES, DELIVERY_DETAILS, …). Both families are
...
No LLM is used anywhere in this report — every section is produced purely
from pandas/matplotlib analysis of the Snowflake rows, so there are no
narrative-generation prompts to preserve.

Deployment adaptation vs. the original standalone script: the source script
wrote the PDF to local disk and left it there for interactive viewing. This
DAG runs on a remote Airflow worker with no guaranteed persistent/browsable
local disk, so the final task instead uploads the finished PDF to S3
(bucket "collabmedbucket", prefix "reports/siaya_medical_camp") via
S3Hook(aws_conn_id="aws_default"). All query logic, cleaning/parsing
helpers, and every report page/chart are otherwise unchanged from the
original script.

Airflow Variables (all optional — a plain run queries the whole table):
  SIAYA_REPORT_SINCE         only rows with VISIT_DATE/DOCUMENT_DATE >= this date (YYYY-MM-DD)
  SIAYA_REPORT_RECORD_TYPE   filter to a single RECORD_TYPE (e.g. V3_EMR_VISIT)
  SIAYA_REPORT_LIMIT         cap number of rows fetched from Snowflake

Airflow Connections required:
  aws_default   S3 credentials for the report-upload step
...
DAG_ID = "siaya_medical_report_analyzer"

# ---------------------------------------------------------------------------
# Snowflake source (mirrors siaya_v2/v3_visits_to_snowflake.py)
# ---------------------------------------------------------------------------
TARGET_DB     = "HOSPITALS"
...
# ---------------------------------------------------------------------------
# S3 destination for the rendered PDF (deployment adaptation — the original
# standalone script just wrote the PDF to local disk)
# ---------------------------------------------------------------------------
S3_CONN_ID = "aws_default"
S3_BUCKET  = "collabmedbucket"
S3_PREFIX  = "reports/siaya_medical_camp"
...
def clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows before any analysis runs on the raw Snowflake export."""
    df = df.copy()
    for c in df.columns:
    ...
    if dropped:
        log.info("Data cleaning: dropped %d exact-duplicate row(s) (%d -> %d)",
                  dropped, before, len(df))
...
def build_report(df: pd.DataFrame, pdf_path: Path) -> None:
    # ---- Normalize columns from the Snowflake table (upper) ----
    df = df.copy()
...
def _snowflake_connect():
    ...
    if pk_path:
        ...
        raise RuntimeError("Set SNOWFLAKE_PRIVATE_KEY_PATH or SNOWFLAKE_PASSWORD in the environment")
...
def fetch_from_snowflake(since: str | None, record_type: str | None, limit: int | None) -> pd.DataFrame:
    ...
    try:
        ...
        log.info("Querying %s.%s.%s (since=%s record_type=%s limit=%s)",
                  TARGET_DB, TARGET_SCHEMA, TARGET_TABLE, since, record_type, limit)
        cur = conn.cursor()
...
# DAG task callables
# ---------------------------------------------------------------------------
def query_data(**context) -> str:
    """Pull the SIAYA_MEDICAL_CAMP_COMPLETE_DATA rows from Snowflake and
    stash them as a parquet file for the next task (tasks in this DAG run
    on the same LocalExecutor worker, so a shared /tmp path is sufficient)."""
    since       = Variable.get("SIAYA_REPORT_SINCE", default_var=None)
    record_type = Variable.get("SIAYA_REPORT_RECORD_TYPE", default_var=None)
    limit_raw   = Variable.get("SIAYA_REPORT_LIMIT", default_var=None)
    limit       = int(limit_raw) if limit_raw else None

    df = fetch_from_snowflake(since, record_type, limit)
    log.info("Loaded %d rows x %d columns from %s.%s.%s",
              len(df), len(df.columns), TARGET_DB, TARGET_SCHEMA, TARGET_TABLE)
    if df.empty:
        log.warning("No rows returned from Snowflake for this run")

    run_id = context["run_id"].replace(":", "_").replace("+", "_")
    data_path = f"/tmp/siaya_medical_camp_data_{run_id}.parquet"
    df.to_parquet(data_path, index=False)
    return data_path


def generate_report(**context) -> str:
    """Build the multi-page PDF report from the queried data."""
    ti = context["ti"]
    data_path = ti.xcom_pull(task_ids="query_data")
    df = pd.read_parquet(data_path)

    run_id = context["run_id"].replace(":", "_").replace("+", "_")
    pdf_path = f"/tmp/siaya_medical_camp_report_{run_id}.pdf"
    build_report(df, Path(pdf_path))
    log.info("Wrote PDF report -> %s", pdf_path)
    return pdf_path


def upload_report(**context):
    """Upload the rendered PDF to S3 — the DAG runs on a remote worker with
    no guaranteed persistent/browsable local disk, so (unlike the original
    standalone script, which just left the PDF on local disk) the final
    artifact is pushed to S3 instead."""
    ti = context["ti"]
    pdf_path = ti.xcom_pull(task_ids="generate_report")

    ds = context["ds"]
    run_id = context["run_id"].replace(":", "_").replace("+", "_")
    key = f"{S3_PREFIX}/{ds}/siaya_medical_camp_report_{run_id}.pdf"

    S3Hook(aws_conn_id=S3_CONN_ID).load_file(
        filename=pdf_path, key=key, bucket_name=S3_BUCKET, replace=True,
    )
    log.info("Uploaded report to s3://%s/%s", S3_BUCKET, key)
