...
Standalone Python script (no Airflow) that:
  ...
  4. IMPORTANT -- based on inspecting a real sample (B6.pdf): these Siaya
     ...
     start page (looking for the printed "OUTPATIENT CLINICAL CONSULTATION
     FORM" / "1. Patient Demographics" header), groups pages into per-patient
     records, and runs ADE Extract ONCE PER RECORD. A single-patient PDF just
     becomes a file with one record, so this also works for normal documents.
...
Optional environment variables:
    ...
    RECORD_START_MARKERS           Comma-separated lowercase substrings that mark
                                    the start of a new record's first page
                                    (default: "outpatient clinical consultation
...
# patient's form starts with the printed header "OUTPATIENT CLINICAL
# CONSULTATION FORM" / "1. Patient Demographics". We detect that header on
# each parsed page and use it to split the page list into per-patient
# groups, so each group gets its own ADE Extract call and its own JSON
# record instead of everything being crammed into a single extraction.
# --------------------------------------------------------------------------
...
def is_record_start_page(markdown: str) -> bool:
    ...
    return any(_normalize(marker) in normalized for marker in RECORD_START_MARKERS)
...
def group_pages_into_records(page_results: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
    ...
    # Fallback: if no page matched the marker (e.g. OCR of the header was too
    # garbled), treat the whole file as a single record rather than dropping
    # everything.
    if not start_indices:
        log.warning(
            "No record-start marker found in any page -- treating the whole "
