...
def run_pipeline(facility: str, *, since: str | None = None,
                 ...
                 page_workers: int = DEFAULT_PAGE_WORKERS) -> None:
    ...
    log.info("══════ START %s · facility=%s · run_id=%s · workers=%d (page_workers=%d) ══════",
             PIPELINE_NAME, facility, run_id, workers, page_workers)
    ...
    # 2-3. Extract → S3 → Snowflake RAW (parallel across jobs, shared SF conn)
    successes: list[dict] = []
    failures:  list[dict] = []
