#!/usr/bin/env python3
"""
orthopedic_v2_raw_pipeline.py — Afya Extraction API gateway → S3 → Snowflake ORTHOPEDIC_RAW_V2

Sibling pipeline to orthopedic_api_to_snowflake.py, built the same way, for a
SECOND connection (connection_id=20) covering the 19 clinical-encounter model
namespaces below (admission notes, triage, requests, results, the patient
master table, etc.) into HOSPITALS.ORTHOPEDIC_RAW_V2 per-model tables.

Flow (identical machinery to the v1 pipeline):
  1. POST https://afyapi.afyaanalytics.ai/api/auth/login  →  cache bearer token (50-min TTL).
  2. For each of the 19 configured namespaces, POST /api/gateway with
  ...
  5. COPY INTO HOSPITALS.ORTHOPEDIC_RAW_V2.<model_table> from the S3 object.
  6. Resume-aware: completed jobs are checkpointed in .orthopedic_v2_progress.json;
     watermarks (.orthopedic_v2_watermarks.json) only advance on a zero-failure run.

GOVERNANCE — WHY THIS LAYER IS *NOT* DE-IDENTIFIED
  This RAW_V2 schema is a full-fidelity landing zone, deliberately unmasked,
  because de-identifying twice (once here, once in the clean layer) risks the
  two layers drifting and makes reprocessing/backfills lossy. Per data
  ...
  lineage intact":
    - HOSPITALS.ORTHOPEDIC_RAW_V2 must be access-restricted to the ETL
      service role ONLY (grant to the pipeline's Snowflake role; do NOT grant
      to analyst/BI roles). This mirrors how you'd lock down a bronze layer.
    - HOSPITALS.ORTHOPEDIC_CLEAN_V2 (built by orthopedic_v2_clean_pipeline.py)
      is the de-identified, analyst-facing layer. Direct identifiers (patient
      name, phone, email, national ID, next-of-kin contacts, address, etc.)
      are hashed or redacted there — see PII_CLASSIFICATION in
      orthopedic_v2_schema_discovery.py / pii_classification_v2.json.
  If your org's policy instead requires masking-at-source (gateway-side
  anonymize/anonymize_fields, same as orthopedic_api_to_snowflake.py does for
  a few v1 models), every MODELS entry below already supports "anonymize",
  "anonymize_fields", "anonymize_skip", and "row_transform" keys — just add
  them per model the same way v1 does, once you've confirmed via
  orthopedic_v2_schema_discovery.py which fields the gateway can safely mask
  (some field shapes — nested arrays, etc. — 500 the anonymizer; v1 learned
  this the hard way, see _redact_textarea_fields / _redact_supplier_contacts
  in orthopedic_api_to_snowflake.py).

USAGE
  python orthopedic_v2_raw_pipeline.py
  python orthopedic_v2_raw_pipeline.py --models patients,triage,admnotes
  python orthopedic_v2_raw_pipeline.py --since 2025-01-01T00:00:00Z
  python orthopedic_v2_raw_pipeline.py --full-refresh
  python orthopedic_v2_raw_pipeline.py --dry-run
  python orthopedic_v2_raw_pipeline.py --no-resume
  python orthopedic_v2_raw_pipeline.py --workers 6 --page-workers 4

ENV VARS  (place in .env next to this script — same .env as orthopedic_api_to_snowflake.py
           works fine, this pipeline only adds its own CONNECTION_ID/FACILITY_ID overrides)
  # Afya Extraction credentials
  AFYA_EXTRACTION_USERNAME=admin
  AFYA_EXTRACTION_PASSWORD=Afya@extract26

  # Gateway config — this pipeline's connection/facility
  AFYA_EXTRACTION_BASE_URL=https://afyapi.afyaanalytics.ai/api
  AFYA_EXTRACTION_V2_CONNECTION_ID=20
  AFYA_EXTRACTION_V2_FACILITY_ID=47
  ...
  PAGE_WORKERS=16        # parallel pages within a job (default 16)
...
log = logging.getLogger("orthopedic_v2_raw_pipeline")
if not log.handlers:
...
PIPELINE_NAME = "orthopedic_v2_api_to_snowflake_raw"
...
# NOTE: this is a distinct connection from orthopedic_api_to_snowflake.py's
# CONNECTION_ID=16 / FACILITY_ID=47. Confirmed with Martin: same facility (47),
# new connection_id (20) covering the 19 models below.
...
WATERMARK_FILE = Path(__file__).resolve().parent / ".orthopedic_v2_watermarks.json"
PROGRESS_FILE  = Path(__file__).resolve().parent / ".orthopedic_v2_progress.json"
...
# ─── MODEL REGISTRY ──────────────────────────────────────────────────────────
# Maps PHP namespace → Snowflake table name in ORTHOPEDIC_RAW_V2.
# Table names below are exactly as specified by Martin (namespace, table).
#
# No anonymize / anonymize_fields / row_transform are set by default — see the
# GOVERNANCE note in the module docstring for why (masking happens once, in
# orthopedic_v2_clean_pipeline.py). Add gateway-side masking here per model
# only after orthopedic_v2_schema_discovery.py confirms the gateway can
# safely mask that model's fields (some shapes 500 the anonymizer — see v1's
# _redact_textarea_fields / _redact_supplier_contacts for precedent).
...
#
# Same layout/semantics as orthopedic_api_to_snowflake.py — see that file's
# comments for the full state-machine description. Short version:
#   status="done"    → skip entirely
#   status="partial" → fetch only missing pages, one COPY INTO covering all
#                      s3_keys (old + new) once complete
...
def _extract_rows_and_pagination(payload: dict) -> tuple[list, dict]:
    ...
    Handles nested shapes where data is {"data": {"data": [...], "current_page": 1, ...}}.
    """
...
def extract_one_model(
    ...
    *,
    ...
    Resume (resume=True):
      ...
      - status='partial' → skip already-uploaded pages, fetch only missing
        pages, then do ONE COPY INTO covering ALL s3_keys (saved + new).
    """
    ...
    if last_page is not None:
        ...
        if remaining:
            ...
            def _do_page(p: int) -> tuple[int, str | None, int]:
                ...
                key = _upload_page(p, rows, last_page)
...
def ensure_schema_and_tables(sf: SnowflakeClient) -> None:
    """Create ORTHOPEDIC_RAW_V2 schema and all per-model tables if they don't exist."""
    sf.execute(
    ...
    log.warning(
        ...
        "This schema is full-fidelity / NOT de-identified. Analysts should "
        "read from %s.ORTHOPEDIC_CLEAN_V2 instead.",
        SF_DB, SF_RAW_SCHEMA, SF_DB,
    )
...
def run_pipeline(
    ...
    *,
    ...
    run_id     = datetime.now(timezone.utc).strftime("orthv2__%Y-%m-%dT%H-%M-%SZ")
    ...
    log.info(
        "══ START %s · run=%s · connection_id=%d facility_id=%d · %d/%d models · workers=%d (pages=%d) ══",
        PIPELINE_NAME, run_id, CONNECTION_ID, FACILITY_ID, len(pending), len(models), workers, page_workers,
    )
...
def main() -> None:
    ...
    ap.add_argument(
        "--models",
        help=(
            "Comma-separated table names to extract, e.g. patients,triage,admnotes. "
            "Uses table names (not namespaces). Omit to run all 19 models."
        ),
    )
    ap.add_argument(
        "--since",
    ...
    ap.add_argument(
        ...
        help="Ignore .orthopedic_v2_progress.json and re-run every model from scratch.",
    )
    ap.add_argument(
    ...
    if args.list_models:
        ...
        print("-" * 55)
        for m in MODELS:
            print(f"{m['namespace']:<30}  {m['table']}")
        print(f"\n{len(MODELS)} models total. connection_id={CONNECTION_ID} facility_id={FACILITY_ID}")
        return
